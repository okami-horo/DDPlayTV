...
class DanmuQueryImpl : DanmuQuery {
    override suspend fun match(hash: String): DanmuEpisodeData? {
        val result =
            ResourceRepository.matchDanmu(hash).getOrNull()
                ?: return null
    ...
    override suspend fun source(episodeId: String): List<DanmuRelatedUrlData> =
        ResourceRepository
            .getRelatedDanmu(episodeId)
            .getOrNull()
            ?.relateds
            ?: emptyList()

    override suspend fun getContentByEpisodeId(
        episodeId: String,
        withRelated: Boolean
    ): List<DanmuContentData> =
        ResourceRepository
            .getDanmuContent(episodeId, withRelated)
            .getOrNull()
            ?.comments
            ?: emptyList()
