...
class DecompressInterceptor : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val originalEncoding =
            originalResponse.header("Content-Encoding")
                ?: return originalResponse

        val decompressEncoding =
            DecompressEncoding.formEncoding(originalEncoding)
                ?: return originalResponse
    ...
    private enum class DecompressEncoding(
        val encoding: String
    ) {
        GZIP("gzip"),
