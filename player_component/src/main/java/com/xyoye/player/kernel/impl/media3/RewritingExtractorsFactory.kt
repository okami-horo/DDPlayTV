...
private class RewritingExtractor(
    private val upstream: Extractor
) : Extractor {
    ...
    ): Int = upstream.read(input, seekPosition)

    override fun seek(
        position: Long,
        timeUs: Long
    ) = upstream.seek(position, timeUs)
...
private class RewritingTrackOutput(
    private val downstream: TrackOutput
) : TrackOutput {
    ...
    override fun sampleData(
        ...
        sampleDataPart: Int
    ): Int = downstream.sampleData(input, length, allowEndOfInput, sampleDataPart)
