...
class SubtitleOverlayView
    ...
    constructor(
        ...
        private val playerLayoutListener =
        ...
        private fun traceCadence(
            nowNs: Long,
            drawDurationNs: Long,
            previousRenderNs: Long
        ) {
            ...
            if (renderCount % 30 == 0) {
                ...
                LogFacade.d(
                    LogModule.PLAYER,
                    LOG_TAG,
                    "cadence frame=$renderCount interval=${"%.2f".format(
                        sinceLastMs,
                    )}ms draw=${"%.2f".format(drawMs)}ms window=${"%.2f".format(traceWindowMs)}ms",
                )
            }
        }
