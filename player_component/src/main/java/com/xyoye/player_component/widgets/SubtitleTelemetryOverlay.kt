 ...
 */
class SubtitleTelemetryOverlay @JvmOverloads constructor(
    context: Context,
    ...
    private val metricsView = TextView(context).apply {
        setTextColor(Color.WHITE)
        textSize = 12f
        layoutParams = LayoutParams(
            LayoutParams.WRAP_CONTENT,
    ...
    fun updateSnapshot(snapshot: TelemetrySnapshot?) {
        ...
        builder.append("Frames: ").append(snapshot.renderedFrames)
            .append(" rendered / ").append(snapshot.droppedFrames).append(" dropped\n")
        builder.append("Vsync hit: ")
    ...
    private fun defaultFilter(sample: TelemetrySample): Boolean {
        return sample.frameStatus != SubtitleFrameStatus.Rendered ||
