...
class LocalSubtitlePipelineApi(
    ...
    override suspend fun init(request: PipelineInitRequest): SubtitlePipelineState {
        outputTarget = SubtitleOutputTarget(
        ...
        val newState = SubtitlePipelineState(
    ...
    override suspend fun status(): PipelineStatusResponse {
        return PipelineStatusResponse(state, outputTarget)
    }

    override suspend fun fallback(command: FallbackCommand): SubtitlePipelineState {
        val previous = state
        val updated = (previous ?: SubtitlePipelineState(
            mode = SubtitlePipelineMode.GPU_GL,
            status = SubtitlePipelineStatus.Initializing,
            surfaceId = null
        )).copy(
            mode = command.targetMode,
            status = when (command.targetMode) {
            ...
            lastRecoverAt = if (command.targetMode == SubtitlePipelineMode.GPU_GL) System.currentTimeMillis() else previous?.lastRecoverAt
        )
        state = updated
    ...
    override suspend fun latestTelemetry(): TelemetrySnapshot? {
        return telemetryRepository.latestSnapshot()
    }
}
