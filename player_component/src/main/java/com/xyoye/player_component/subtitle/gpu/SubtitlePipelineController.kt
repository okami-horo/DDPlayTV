...
class SubtitlePipelineController(
    ...
    ): SubtitlePipelineState = withContext(dispatcher) {
        cachedSurfaceId.set(surfaceId)
        val request = PipelineInitRequest(
            surfaceId = surfaceId,
    ...
    suspend fun status(): PipelineStatusResponse = withContext(dispatcher) {
    ...
    suspend fun latestTelemetry() = withContext(dispatcher) {
        api.latestTelemetry()
    }
