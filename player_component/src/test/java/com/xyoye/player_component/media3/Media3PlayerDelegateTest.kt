...
private class FakeMedia3Dao : Media3Dao {
    ...
    override suspend fun recentSnapshots(limit: Int): List<RolloutToggleSnapshot> = snapshots.take(limit)

    override suspend fun upsertDownloadCheck(check: DownloadAssetCheck) {
        downloads[check.downloadId] = check
    }

    override suspend fun findDownloadCheck(downloadId: String): DownloadAssetCheck? = downloads[downloadId]
