...
- FR-014（样式优先级）
  1. Given 启用 libass，When 在“字幕样式设置”修改文字大小/颜色/描边/阴影，Then 对渲染结果无影响；修改“时间偏移/垂直偏移”时于下一帧生效（≤16ms），播放不中断。
...
- FR-015（SurfaceView 叠加实现细则）
  ...
  2. Given `SurfaceType` 为 SurfaceView 且启用 libass，When 发生旋转/分屏/窗口大小变化，Then 字幕适配新的可见区域，定位与对齐正确（适配时限以 FR-013 为准）。
...
- libass 支持：ASS/SSA（走 libass 渲染路径）。
- 其他常见格式（如 SRT 等）：默认走“自定义渲染后端（Canvas/TextPaint）”。
- 当选择 libass 而当前字幕格式不受支持时：按照 FR-008 提示用户是否切回旧后端；默认不变更设置且继续播放（该字幕可不渲染）。

### Non-Functional Requirements

- 性能目标：在常规设备上保持 60fps 画面渲染；字幕渲染不应成为主要掉帧来源。
- 首帧可见：切换后端后第一次进入播放，字幕首帧可见时间不劣于现有实现的 ±10%。
- 故障可用性：从检测到初始化/渲染失败到阻断式提示可见的时间 ≤ 2s；用户确认“切回旧后端”后，字幕恢复可见 ≤ 2s。
- 内存边界：字幕位图缓冲总占用不超过“可见区域 RGBA 位图 2 帧”上限；应尽量复用单一 ARGB 缓冲。
- 重绘门限：仅在 libass 标记 changed==1 或时间游标进入下一事件时重绘；在 CPU/内存压力下可自适应降低重绘频率，确保交互与视频播放优先级。
- 可观测性：为 libass 路径打点帧间渲染节奏、首帧延迟、错误原因与回退事件；日志不包含敏感内容。
...
- **SC-001**: 通过包含复杂样式/多语言/高密度场景的标准测试样本集，至少 90% 的样本在 libass 渲染后端下被评为“与参考渲染视觉等效”。评测采用双盲或对照基线方法：固定样本集（含 ASS/SSA 特效与高密度弹幕场景），由两名以上评审独立打分，按一致性阈值计算通过率。
- **SC-002**: 用户可在设置中 10 秒内找到并完成渲染后端切换；切换后首次进入播放的字幕首帧可见时间不劣于现有实现 ±10%。
 - **SC-003**: 故障到提示出现 ≤ 2s，确认切回到字幕恢复可见 ≤ 2s（采样于 5 次以上试验并取中位数）。
