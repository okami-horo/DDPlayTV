...
2. **Given** 系统已展示二维码且用户使用官方客户端扫码并确认授权，**When** 授权流程完成，**Then** 系统创建该存储源且展示网盘根目录/默认目录的文件列表
...
- **FR-002**: 系统必须提供“二维码扫码授权登录”方式来连接百度网盘，且不得要求用户输入百度账号密码。
- **FR-003**: 系统必须在授权成功后展示网盘中的目录与文件，并支持进入/返回目录层级浏览。
...
- **FR-002**: 连接流程展示可扫码二维码且不出现“百度账号/密码”输入项；用户可取消、刷新二维码、重试，且每种结果均有明确提示。
- **FR-003**: 授权成功后可看到至少一个目录层级的列表；进入与返回目录后列表内容正确且可继续操作。
...
- **百度网盘存储源**: 代表一个已添加的百度网盘连接（展示名、账号标识、授权状态、最近一次成功访问时间）。
