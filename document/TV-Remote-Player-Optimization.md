...
- DPAD 行为（播放中、非弹出模式）
  ...
  - `LEFT/RIGHT`：当控制器未显示时，按次固定快退/快进 10s（`TvVideoController.changePosition(±10s)`）。若当前控制器已显示，仅唤起控制器并不消费事件（交由 Setting）。
...
- 明确“状态 × 按键 → 行为”矩阵（控制器隐藏/显示、设置面板、悬浮窗）；
...
- 播放态（控制器隐藏）
  - `DPAD_CENTER`：播放/暂停切换。
  - `DPAD_LEFT/RIGHT`：固定步长快退/快进 10s（保持现状，不引入长按加速与可配置步长）。
  - `DPAD_UP`：打开“选集”面板，焦点定位到当前集或下一集，便于快速切换上下集。
...
- 维护 `UiState`（ControllerHidden / ControllerVisible / Settings / Popup）；
...
- 个别遥控器没有 Menu 键：无需额外适配。按 `DPAD_DOWN` 唤起控制条后，通过焦点移动至控制条中的设置按钮进入设置即可；
