...
object SubtitleFontManager {
    ...
    private const val LEGACY_FONT_DIR_NAME = "fonts"
    ...
    private fun ensureFontsDirectory(context: Context): File {
        val directory = PathHelper.getFontDirectory()
        migrateLegacyFontsIfNeed(context, directory)
        if (!directory.exists()) {
    ...
    private fun copyDefaultFontFromAssets(
        ...
        return runCatching {
            ...
            LogFacade.e(
                LogModule.SUBTITLE,
                TAG,
                "copy default font failed: ${it.message}",
                throwable = it,
            )
        }.getOrDefault(false)
    }
    ...
    private fun migrateLegacyFontsIfNeed(
        ...
        synchronized(this) {
            ...
            val legacyDir = File(context.filesDir, LEGACY_FONT_DIR_NAME)
            ...
            legacyDir.listFiles()?.forEach { legacyFile ->
                ...
                runCatching { legacyFile.inputStream().use { it.copyTo(targetFile.outputStream()) } }
                    .onSuccess {
                        LogFacade.i(
                            LogModule.SUBTITLE,
                            TAG,
