...
object PathHelper {
     ...
     * 获取保存弹幕的文件夹
     */
    fun getDanmuDirectory(): File = getCacheDirectory(CacheType.DANMU_CACHE)

    /**
     * 获取保存字幕的文件夹
     */
    fun getSubtitleDirectory(): File = getCacheDirectory(CacheType.SUBTITLE_CACHE)
     ...
     */
    fun getVideoCoverFile(key: String): File = File(getVideoCoverDirectory(), CacheKeyMapper.toSafeFileName(key))
     ...
     * 获取播放的临时缓存文件夹
     */
    fun getPlayCacheDirectory(): File = getCacheDirectory(CacheType.PLAY_CACHE)

    /**
     * 获取播放截图的缓存文件夹
     */
    fun getScreenShotDirectory(): File = getCacheDirectory(CacheType.SCREEN_SHOT_CACHE)

    /**
     * 获取字幕字体缓存文件夹
     */
    fun getFontDirectory(): File = getCacheDirectory(CacheType.FONT_CACHE)

    /**
     * 获取缓存文件文件夹
     */
    fun getCacheDirectory(type: CacheType): File =
        File(getCachePath(), type.dirName).apply {
            checkDirectory(this)
        }
